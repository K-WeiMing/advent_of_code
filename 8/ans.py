...
def task_1(file):
    ...
    visible_trees = (
        len(grid) * 2 + (len(grid[0]) - 2) * 2
    )  # Get number of trees outside
    for row_index, row in enumerate(grid):
        if row_index == 0 or row_index == len(grid) - 1:  # Skip first and last row
            continue
        for tree_index, tree_height in enumerate(row):
            if (
                tree_index == 0 or tree_index == len(row) - 1
            ):  # Skip first and last column
                continue
            tree_height = int(tree_height)
            visible_from_left = all(
                int(height) < tree_height for height in row[:tree_index]
            )
            visible_from_right = all(
                int(height) < tree_height for height in row[tree_index + 1 :]
            )
            ...
            visible_from_bottom = all(
                int(height) < tree_height for height in bottom_trees
            )
            visible_trees += any(
                [
                    visible_from_left,
...
def get_vision(tree_height, tree_list):
    ...
    while index != len(tree_list):
        vision += 1
        if (
            int(tree_list[index]) >= tree_height
        ):  # If tree is the same height or higher, then it's the last visible one
            return vision
        index += 1
    return vision
